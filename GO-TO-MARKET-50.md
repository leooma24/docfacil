# Plan Go-To-Market: Primeros 50 Clientes Pagados de DocFácil

> Generado: 2026-04-11 | Horizonte: 12 semanas (abril-julio 2026)
> Base: Culiacán + Los Mochis, Sinaloa | Equipo: 2 personas (dev + socio comercial)
> Producto: Listo en producción | Presupuesto: Bootstrapped

---

## RESUMEN EJECUTIVO

**Meta:** 50 clientes pagados en 12 semanas = ~4.2 clientes/semana promedio.

Realidad: las primeras 4 semanas serán de carga del pipeline (1-2 cierres/semana), las semanas 5-12 de cosecha (5-7 cierres/semana). La curva es una J, no una línea recta.

**Distribución geográfica objetivo:**
- Culiacán: 30 clientes (ciudad más grande, base de operaciones)
- Los Mochis: 12 clientes (ICP ya validado, segunda ciudad)
- Mazatlán: 5 clientes (turismo = dentistas cosméticos con presupuesto)
- Otras ciudades de Sinaloa: 3 clientes (Guasave, Guamúchil, Navolato)

---

## 1. PLAN SEMANA POR SEMANA (12 SEMANAS)

### FASE 1: CARGA DEL PIPELINE (Semanas 1-4)

---

#### SEMANA 1 — "Construir la lista y salir a la calle"

**Socio Comercial (SC):**
- Lunes-Martes: Abrir Google Maps, buscar "dentista Culiacán", "consultorio dental Culiacán", "ortodoncista Culiacán". Extraer a un Google Sheet: nombre del doctor, nombre del consultorio, dirección, teléfono, rating, # reseñas, si tiene WhatsApp Business. Meta: 100 consultorios de Culiacán.
- Miércoles: Repetir para Los Mochis (ya tienen el ICP con 267 dentistas). Meta: 80 consultorios.
- Jueves-Viernes: Recorrido presencial por las 5 zonas de mayor densidad de consultorios en Culiacán:
  1. Col. Centro (Ángel Flores, Obregón, Rosales)
  2. Col. Guadalupe (Blvd. Universitarios)
  3. Desarrollo Urbano Tres Ríos
  4. Col. Las Quintas
  5. Chapultepec / Isla Musala
- En cada consultorio: dejar volante con QR + hablar con la recepcionista. NO pedir hablar con el doctor si está en consulta. Pedir el WhatsApp del doctor o de la recepcionista. Anotar: nombre recepcionista, # sillones visibles, si usa computadora en recepción.
- Meta: visitar 25 consultorios físicos, obtener 15 WhatsApp.
- Viernes PM: Subir todos los contactos al panel de ventas de DocFácil.

**Fundador Técnico (FT):**
- Lunes: Crear cuenta demo con datos realistas (20 pacientes ficticios, agenda con citas, recetas generadas) para mostrar en demos. La demo debe verse "vivida", no vacía.
- Martes: Grabar video demo de 3 minutos (pantalla + voz) mostrando: agendar cita → recordatorio WhatsApp → cobro → receta PDF. Subir a YouTube (no listado) y tener link listo para WhatsApp.
- Miércoles: Crear 3 volantes PDF/impresos (tamaño media carta):
  - Volante A: "¿Tus pacientes faltan a sus citas? Recordatorios automáticos por WhatsApp" + QR a landing
  - Volante B: "Tu consultorio digital desde $16 al día" + QR + teléfono WhatsApp
  - Volante C: "Expediente clínico + Recetas + Cobros en un solo lugar" + QR
- Jueves: Imprimir 200 volantes (costo: ~$300 MXN en impresión digital). Mandar hacer 100 tarjetas de presentación con QR al demo ($150 MXN).
- Viernes: Crear 5 mensajes plantilla de WhatsApp para outreach frío (ver sección de templates abajo). Configurar respuestas rápidas en WhatsApp Business.

**KPIs Semana 1:**
| Métrica | Meta |
|---|---|
| Consultorios en base de datos | 180 |
| Visitas presenciales | 25 |
| WhatsApp obtenidos | 15 |
| Video demo listo | 1 |
| Volantes impresos | 200 |
| Demos agendadas | 3 |
| Trials activados | 0 |
| Clientes pagados | 0 |

---

#### SEMANA 2 — "Outreach masivo + primeras demos"

**SC:**
- Lunes-Martes: Enviar WhatsApp frío a los 100 contactos de Culiacán. Ritmo: 30/día (para no ser marcado como spam). Mensaje personalizado con nombre del doctor y referencia a su consultorio (ej: "Vi su consultorio en Blvd. Rosales...").
- Miércoles: Seguimiento (segundo mensaje) a los que dejaron en visto. Mensaje corto: "Dr. [nombre], ¿le puedo mandar un video de 3 min mostrando cómo funciona? Sin compromiso."
- Jueves-Viernes: Dar las 3+ demos agendadas. Formato:
  - Presencial si es en Culiacán (llevar laptop, mostrar en la computadora de recepción si se puede)
  - VideoLlamada WhatsApp si es Los Mochis (compartir pantalla)
  - Duración: 15 minutos MAX. Mostrar solo: agenda → recordatorio → cobro → receta
  - Al final: "¿Quiere probarlo gratis 14 días con sus propios pacientes?"
- Viernes PM: Registrar resultados en panel de ventas. Clasificar: caliente / tibio / frío / descartado.

**FT:**
- Lunes: Publicar artículo en blog: "5 señales de que tu consultorio dental necesita un software" (SEO: "software para consultorio dental México"). 800 palabras, incluir CTA al trial.
- Martes: Crear la página /precio con tabla comparativa clara: Gratis vs Básico vs Pro vs Clínica. Incluir botón de WhatsApp en cada plan.
- Miércoles: Unirse a 5 grupos de Facebook de dentistas:
  1. "Odontólogos de Sinaloa"
  2. "Dentistas México - Casos Clínicos"
  3. "Odontología General México"
  4. "Material Dental México - Compra y Venta"
  5. "Dentistas Emprendedores MX"
  - NO vender. Solo contestar preguntas, dar valor. Poner en el perfil "Co-fundador de DocFácil".
- Jueves: Preparar onboarding automatizado: cuando un doctor se registra al trial, que reciba WhatsApp automático: "¡Bienvenido! Soy [nombre]. En 10 minutos puedo ayudarte a configurar tu primer paciente y cita. ¿Cuándo te queda bien?"
- Viernes: Soporte a los trials activos. Asegurar que cada trial tenga al menos 1 paciente y 1 cita creada antes del fin de semana.

**KPIs Semana 2:**
| Métrica | Meta |
|---|---|
| WhatsApp enviados | 100 |
| Respuestas recibidas | 25 (25%) |
| Demos realizadas | 5 |
| Trials activados | 3 |
| Clientes pagados | 0-1 |

---

#### SEMANA 3 — "Segundo round presencial + Los Mochis"

**SC:**
- Lunes: Segundo recorrido presencial en Culiacán, zonas diferentes a semana 1:
  - Colonia Chapultepec
  - Plaza Fiesta (zona médica)
  - Blvd. Emiliano Zapata
  - Meta: 20 consultorios nuevos
- Martes: WhatsApp outreach a 50 consultorios de Los Mochis (del ICP ya construido).
- Miércoles: Seguimiento a TODOS los tibios de semana 2. Llamada telefónica (no solo WhatsApp). Guion: "Doctor, le marqué porque vi que le interesó DocFácil la semana pasada. ¿Tiene 5 minutos para que le resuelva cualquier duda?"
- Jueves: Demos. Meta: 5 demos en la semana.
- Viernes: Si hay un trial activo de semana 2, llamar para hacer check-in: "¿Cómo le fue? ¿Necesita ayuda con algo?"
- Sábado (medio día): Visitar 5 consultorios que abren sábado en Culiacán (muchos dentistas trabajan sábado medio día). Hora ideal: 10-12pm.

**FT:**
- Lunes: Publicar en Facebook (desde cuenta personal, no página) un post tipo storytelling: "Mi socio y yo creamos un software para consultorios dentales porque [historia real]. Ya hay doctores usándolo. Si conoces un dentista que lucha con las citas perdidas, etiquétalo." Meta: 5+ etiquetas.
- Martes: Buscar en Google Maps "distribuidor dental Culiacán" y "depósito dental Culiacán". Hacer lista de 5-8 distribuidores. Estos son aliados, no clientes: ellos visitan 50+ consultorios al mes.
- Miércoles: Visitar al distribuidor dental más grande de Culiacán con una propuesta: "Por cada dentista que nos refieras y se suscriba, te damos $200 MXN de comisión o $300 en crédito en DocFácil para tus clientes."
- Jueves: Revisar métricas de la semana en el panel de ventas. Ajustar los mensajes de WhatsApp según qué templates tienen mejor respuesta.
- Viernes: Automatizar el email de seguimiento: 3 días después del trial, enviar email con "3 cosas que puedes hacer hoy en DocFácil" (agendar cita, registrar paciente, generar receta).

**KPIs Semana 3:**
| Métrica | Meta |
|---|---|
| Contactos nuevos agregados | 70 |
| WhatsApp enviados (acumulado) | 220 |
| Demos realizadas (semana) | 5 |
| Demos realizadas (acumulado) | 10 |
| Trials activos | 6 |
| Clientes pagados (acumulado) | 2-3 |

---

#### SEMANA 4 — "Cerrar los tibios + primer referido"

**SC:**
- Lunes-Martes: Blitz de seguimiento. Llamar a TODOS los prospectos tibios (los que vieron demo pero no activaron trial, o activaron trial pero no lo usaron). Mensaje: "Doctor, su prueba gratis se acaba en X días. ¿Quiere que le ayude a migrar sus pacientes? Le puedo cargar sus primeros 50 pacientes gratis."
- Miércoles: Pedir referidos a los 2-3 clientes pagados y a los trials activos: "¿Conoce algún colega que también le serviría? Si se suscribe, le regalamos un mes gratis a usted."
- Jueves: Contactar al Colegio de Cirujanos Dentistas de Culiacán. Pedir hablar con el presidente o secretario. Propuesta: "Queremos presentar DocFácil en su próxima reunión mensual. 15 minutos, sin costo, y para sus agremiados tenemos precio especial."
- Viernes: Demos con referidos + nuevos.

**FT:**
- Lunes: Crear landing page específica: docfacil.com/dentistas-culiacan (si no existe ya). Con testimonial del primer cliente pagado (aunque sea anónimo: "Dentista en Culiacán: 'Reduje mis citas perdidas un 40% en 2 semanas'").
- Martes: Crear WhatsApp Status (estados) con:
  - Screenshot de la interfaz con datos difuminados
  - "Ya somos X consultorios usando DocFácil"
  - Mini-tip: "¿Sabías que el 30% de los pacientes olvidan sus citas si no les mandas recordatorio?"
  - Publicar 3 estados, que se vean a las 12pm (hora pico de WhatsApp).
- Miércoles: Publicar segundo artículo SEO: "Cómo reducir citas perdidas en tu consultorio dental: guía 2026"
- Jueves: Configurar Google Business Profile para DocFácil si no existe. Categoría: "Software company". Esto ayuda a aparecer cuando buscan "software dental Culiacán".
- Viernes: Revisar métricas de las 4 semanas. CHECKPOINT: ¿Vamos en ruta a 50?

**KPIs Semana 4:**
| Métrica | Meta |
|---|---|
| Contactos totales en pipeline | 250+ |
| Demos realizadas (acumulado) | 18 |
| Trials activos | 10 |
| Clientes pagados (acumulado) | 5-7 |
| Referidos recibidos | 3-5 |
| Tasa demo → trial | 60%+ |
| Tasa trial → paid | 30%+ |

**CHECKPOINT SEMANA 4:** Si tienes menos de 4 clientes pagados, ver sección 6 (Riesgos y Contingencias) para ajustar.

---

### FASE 2: ACELERACIÓN (Semanas 5-8)

---

#### SEMANA 5 — "Escalar lo que funciona"

**SC:**
- Lunes: Analizar qué canal generó los 5-7 clientes de las primeras 4 semanas. Duplicar esfuerzo en ese canal.
- Si fueron visitas presenciales: dedicar 3 días/semana completos a visitas. Rutear por zona para maximizar consultorios/hora.
- Si fue WhatsApp frío: escalar a 50 mensajes/día con el template ganador.
- Martes-Jueves: Ejecutar el canal ganador. Meta: 8 demos en la semana.
- Viernes: Viaje de un día a Los Mochis (2 hrs en carro desde Culiacán). Agendar 4-5 demos presenciales con prospectos tibios que no cerraron por WhatsApp. Llevar laptop y volantes.

**FT:**
- Crear caso de éxito con el primer cliente satisfecho. Formato: 1 párrafo + 1 foto del consultorio (con permiso) + 1 quote. Publicar en landing y redes.
- Activar programa de referidos formal en el sistema: código de referido por cliente, descuento automático al referir.
- Publicar 1 reel/video corto en Instagram: "Así se ve agendar una cita en DocFácil" (grabación de pantalla 30 segundos, música de fondo).
- Participar activamente en 2 grupos de Facebook respondiendo preguntas técnicas (posicionarse como experto).

**KPIs Semana 5:**
| Métrica | Meta |
|---|---|
| Demos en la semana | 8 |
| Trials activos (acumulado) | 15 |
| Clientes pagados (acumulado) | 10-12 |
| Caso de éxito publicado | 1 |

---

#### SEMANA 6 — "WhatsApp masivo + contenido social"

**SC:**
- Lunes-Miércoles: Outreach a los contactos restantes de la base (los que no han sido contactados). Meta: 80 WhatsApp nuevos.
- Jueves: Buscar prospectos en Mazatlán (Google Maps). Meta: 40 consultorios dentales. Mazatlán tiene dentistas cosméticos/turísticos con presupuesto más alto.
- Viernes: Contactar Colegio de Dentistas de Mazatlán y de Los Mochis para presentación.

**FT:**
- Publicar 3 posts en Facebook durante la semana:
  1. Post educativo: "Las 3 funciones que TODO consultorio dental necesita digitalizar primero" (con imagen/carrusel)
  2. Post de prueba social: "Esta semana nos eligieron 4 consultorios nuevos en Sinaloa. Gracias por confiar."
  3. Post interactivo: "Doctores: ¿Cuál es su mayor dolor de cabeza administrativo? A) Citas perdidas B) Cobros C) Expedientes D) Todo lo anterior"
- Optimizar la landing principal con los aprendizajes de las primeras demos (¿qué es lo primero que preguntan? Ponerlo arriba).
- Configurar una campaña básica de Facebook Ads: $100 MXN/día ($3,000/mes), segmentando: profesionales de salud en Sinaloa, 28-55 años, interés en odontología. Anuncio con video demo de 60 segundos. CTA: WhatsApp directo.

**KPIs Semana 6:**
| Métrica | Meta |
|---|---|
| WhatsApp enviados (semana) | 80 |
| Contactos de Mazatlán | 40 |
| Demos (semana) | 6 |
| Clientes pagados (acumulado) | 15-17 |
| Facebook Ads lanzados | 1 campaña |

---

#### SEMANA 7 — "Alianzas + evento local"

**SC:**
- Lunes: Seguimiento a distribuidores dentales. ¿Han referido a alguien? Refrescar la relación. Llevarles más volantes.
- Martes: Contactar la Facultad de Odontología de la UAS (Universidad Autónoma de Sinaloa) en Culiacán. Propuesta: ofrecer DocFácil gratis a residentes/pasantes como herramienta de aprendizaje. Los residentes de hoy son los dentistas que abren consultorio en 1-2 años.
- Miércoles: Presentación en el Colegio de Dentistas (si se logró agendar en semana 4). Formato:
  - 5 min: "El problema de las citas perdidas en México" (estadísticas)
  - 5 min: Demo en vivo de DocFácil
  - 5 min: Oferta especial para agremiados ($499/mes en plan Básico, 6 meses gratis si entran al programa Fundador)
  - Recoger WhatsApp de todos los asistentes
- Jueves-Viernes: Demos con leads del evento + leads de Facebook Ads.

**FT:**
- Preparar la presentación para el Colegio (slides en Google Slides, máximo 10 slides).
- Si Facebook Ads está generando leads, optimizar: pausa los anuncios con CTR <1%, escala los que tienen CTR >2%.
- Publicar artículo SEO: "Mejor software para consultorios dentales en México 2026: comparativa" (targetear a los que buscan competidores).
- Configurar pixel de Facebook en la landing para retargeting futuro.

**KPIs Semana 7:**
| Métrica | Meta |
|---|---|
| Asistentes a presentación Colegio | 15-20 |
| Leads del evento | 8-10 |
| Demos (semana) | 8 |
| Clientes pagados (acumulado) | 20-23 |

---

#### SEMANA 8 — "Mazatlán + cierre de mes"

**SC:**
- Lunes-Martes: Viaje a Mazatlán (4 hrs desde Culiacán). Día 1: visitar 10 consultorios en zona turística/centro. Día 2: demos presenciales agendadas previamente por WhatsApp.
- Miércoles: Día de seguimiento intensivo. Llamar a TODOS los trials que llevan 7+ días sin actividad. "Doctor, vi que no ha tenido chance de usar DocFácil. ¿Qué le falta? ¿Quiere que yo mismo le cargue sus pacientes?"
- Jueves-Viernes: Cerrar deals pendientes. Ofrecer a los indecisos: "Si se suscribe esta semana, le incluyo la migración de datos gratis (valor $1,500)."

**FT:**
- Dashboard de ventas: revisar funnel completo. ¿Dónde se atoran? ¿En demo→trial o en trial→paid? Ajustar.
- Si trial→paid es bajo (<25%): el problema es el onboarding. Crear guía paso a paso con screenshots: "Tus primeros 30 minutos en DocFácil".
- Si demo→trial es bajo (<50%): el problema es la demo. Cambiar el pitch: enfocarse en el dolor #1 que mencionan los doctores.
- Publicar post de "cierre de mes": "Abril fue increíble. Ya somos X consultorios digitalizándose con DocFácil."

**KPIs Semana 8 (MITAD DE CAMINO):**
| Métrica | Meta |
|---|---|
| Consultorios visitados en Mazatlán | 10 |
| Demos (semana) | 7 |
| Clientes pagados (acumulado) | **25-28** |
| MRR cobrado | $0 (los clientes pagados son fundadores dentro de sus 6 meses gratis; empiezan a pagar $499/mes en el mes 7) |
| Trials activos sin convertir | <8 (seguimiento agresivo) |

---

### FASE 3: COSECHA Y ESCALA (Semanas 9-12)

---

#### SEMANA 9 — "Referidos como motor principal"

**SC:**
- Lunes: Contactar a los 25+ clientes pagados. Pedir referido a CADA UNO. Mensaje: "Doctor, ¿cómo le ha ido con DocFácil? Me da gusto. Oiga, ¿tiene algún colega que también le serviría? Si se suscribe, le regalamos un mes a usted."
- Martes-Miércoles: Contactar todos los referidos. Los referidos tienen 3x la tasa de conversión del outreach frío. Prioridad máxima.
- Jueves-Viernes: Outreach a ciudades secundarias: Guasave (30 min de Los Mochis), Guamúchil, Navolato. Google Maps + WhatsApp frío.

**FT:**
- Implementar NPS simple: a los 30 días de suscripción, enviar WhatsApp: "Del 1 al 10, ¿qué tanto recomendaría DocFácil? Responda con un número." Los que digan 9-10 = pedir referido + testimonial. Los que digan 1-6 = llamar inmediatamente para resolver.
- Crear segundo video: caso de éxito en video (30-60 seg, el doctor diciendo en cámara por qué usa DocFácil). Pagar $500 MXN de regalo si es necesario.
- Evaluar Google Ads: si hay presupuesto ($50-100/día), activar campaña de Search con keywords:
  - "software para consultorio dental"
  - "agenda para dentistas"
  - "recordatorio citas consultorio"
  - "expediente clínico digital"
  - Solo Sinaloa. CPC esperado: $5-15 MXN.

**KPIs Semana 9:**
| Métrica | Meta |
|---|---|
| Referidos solicitados | 25 |
| Referidos recibidos | 8-10 |
| Clientes pagados (acumulado) | 30-33 |

---

#### SEMANA 10 — "Segundo round de eventos + contenido"

**SC:**
- Presentación en segundo Colegio (Los Mochis o Mazatlán).
- Visitar proveedores de insumos dentales que aún no se han contactado.
- Outreach a doctores médicos (no dentales): médicos generales, pediatras, ginecólogos. DocFácil funciona para cualquier consultorio. Esto abre un mercado adyacente sin cambiar el producto.
- Meta: 20 contactos de médicos no-dentales.

**FT:**
- Publicar artículo: "Por qué los consultorios médicos en México siguen usando papel (y cómo cambiar hoy)"
- Crear landing: docfacil.com/medicos-generales (reutilizar la landing de dentistas, cambiar imágenes y copy).
- Revisar churn: ¿algún cliente canceló o no renovó? Llamar inmediatamente. Cada cliente perdido son 10+ contactos necesarios para reemplazarlo.

**KPIs Semana 10:**
| Métrica | Meta |
|---|---|
| Clientes pagados (acumulado) | 35-38 |
| Churn (cancelaciones) | 0-1 |
| Contactos médicos no-dentales | 20 |

---

#### SEMANA 11 — "Sprint final: cerrar pipeline"

**SC:**
- Lunes: Lista de TODOS los prospectos en pipeline que alguna vez mostraron interés pero no cerraron. Contactar a cada uno con oferta de cierre: "Doctor, quedan 3 lugares del programa Fundador: 6 meses gratis y después $499/mes congelado de por vida. Cuando se llenen, ya no hay forma de entrar."
- Martes-Jueves: Demos y cierres intensivos. Meta: 10 demos en 3 días.
- Viernes: Tercer viaje a Los Mochis o Mazatlán para cerrar deals presenciales.

**FT:**
- Soporte intensivo a trials activos para convertirlos antes de que expiren.
- Publicar en todos los grupos de Facebook un post de valor + mención sutil: "Pregunta para dentistas: ¿cuántas citas pierden al mes por no-shows? Nosotros en DocFácil vemos que el promedio es 8-12 al mes. Con recordatorios por WhatsApp baja a 2-3."
- Preparar reporte para inversionistas/socios (si aplica): MRR, clientes, retención, pipeline.

**KPIs Semana 11:**
| Métrica | Meta |
|---|---|
| Demos (semana) | 10 |
| Clientes pagados (acumulado) | 42-46 |

---

#### SEMANA 12 — "Cerrar los 50 + planear Q3"

**SC:**
- Lunes-Miércoles: Cierre de los últimos 4-8 deals. Llamadas directas, visitas si es necesario. Usar urgencia real: "El programa Fundador cierra en cuanto se llenen los 10 lugares."
- Jueves: Celebrar el milestone de 50 clientes. Publicar en redes: "50 consultorios en Sinaloa ya son digitales con DocFácil. Gracias." Etiquetar a los clientes que dieron permiso.
- Viernes: Sesión de planeación Q3:
  - ¿Contratar un vendedor de tiempo completo?
  - ¿Expandir a Sonora/Durango/Nayarit?
  - ¿Invertir más en Google Ads con el MRR generado?
  - ¿Qué features pidieron los 50 clientes? Priorizarlas.

**FT:**
- Automatizar lo que se hizo manual: secuencias de WhatsApp, emails de onboarding, recordatorios de trial expirando.
- Documentar el playbook de ventas que funcionó para replicarlo con un vendedor contratado.
- Planear features del Q3 basadas en feedback de clientes reales.

**KPIs Semana 12:**
| Métrica | Meta |
|---|---|
| **Clientes pagados (acumulado)** | **50** |
| MRR cobrado | **~$31,960 MXN/mes** en la semana 12 (40 clientes pagando × $799 de ARPU; los 10 fundadores siguen en sus 6 meses gratis) → **~$39,950 MXN/mes** al mes 7, cuando los fundadores empiezan a pagar |
| Trials activos pendientes | 5-8 (pipeline para semana 13) |
| Tasa de retención | >90% |
| NPS promedio | >8 |

---

## 2. CANALES DE ADQUISICIÓN (Ordenados por ROI)

### Canal 1: Visitas Puerta a Puerta en Culiacán
**Clientes esperados: 15 de 50**

| Aspecto | Detalle |
|---|---|
| Costo | $0 (gasolina + volantes ~$500 total) |
| Tiempo para resultados | 1-2 semanas |
| Dificultad | Baja (requiere piernas y cara dura) |
| Responsable | Socio comercial |

**Ejecución paso a paso:**
1. Rutear por zona: agrupar consultorios por colonia para visitar 8-10 por recorrido (3 hrs).
2. Llegar al consultorio, hablar con la RECEPCIONISTA primero. "Buenos días, ¿el doctor [nombre] se encuentra? Vengo a mostrarle un sistema para consultorios que les manda recordatorio de citas por WhatsApp a los pacientes."
3. Si el doctor está disponible (raro): demo express de 5 minutos en tu celular.
4. Si no está: dejar volante CON TU NÚMERO DE WHATSAPP escrito a mano. Decir a la recepcionista: "Se lo puede mencionar cuando termine? Es gratis probarlo."
5. Anotar en el celular: nombre recepcionista, hora de visita, si el consultorio se ve moderno o tradicional, cuántos sillones ves.
6. A las 24 hrs: enviar WhatsApp al consultorio: "Buen día, soy [nombre] de DocFácil, ayer pasé por su consultorio y platiqué con [nombre recepcionista]. Le mando un video de 3 min de cómo funciona: [link]"
7. A las 72 hrs sin respuesta: segundo mensaje: "Doctor, sin presión. Solo le comento que puede probarlo gratis 14 días. Si le interesa, aquí estoy. Si no, con gusto. Que tenga buen día."

**Por qué funciona:** En México, especialmente en ciudades como Culiacán, la venta presencial genera confianza. Los doctores son inundados de WhatsApp spam pero reciben pocas visitas profesionales. Ver una cara genera 3x más confianza que un mensaje digital.

**Números:** 100 visitas → 40 que reciben el mensaje de seguimiento → 15 demos → 10 trials → 5-6 paid. Hacer 200 visitas en 12 semanas = 10-15 clientes.

---

### Canal 2: WhatsApp Frío con Directorio
**Clientes esperados: 12 de 50**

| Aspecto | Detalle |
|---|---|
| Costo | $0 |
| Tiempo para resultados | 1 semana |
| Dificultad | Media (requiere buen copywriting) |
| Responsable | Socio comercial |

**Ejecución paso a paso:**
1. Fuentes de números: Google Maps, Doctoralia, Sección Amarilla, DENUE (denue.inegi.org.mx).
2. Regla de oro: MÁXIMO 30 mensajes nuevos por día desde un número de WhatsApp. Más de eso = riesgo de ban.
3. Usar WhatsApp Business (no personal). Configurar perfil con logo, horario, catálogo con los planes.
4. Enviar desde las 12:00-14:00 hrs (entre pacientes de mañana y tarde) o 18:00-19:00 (al cerrar el consultorio).
5. Nunca enviar a las 8am ni después de las 9pm.

**Templates probados (usar en rotación):**

**Template A — El Directo:**
> Buen día Dr. [nombre], soy [tu nombre] de DocFácil. Hacemos software para consultorios dentales: agenda con recordatorios automáticos por WhatsApp a sus pacientes, expediente digital y cobros. Desde $499/mes. ¿Le puedo mandar un video de 3 min mostrando cómo funciona?

**Template B — El Problema:**
> Dr. [nombre], ¿le pasa que los pacientes faltan a sus citas sin avisar? Un software de recordatorios automáticos por WhatsApp reduce las citas perdidas hasta un 50%. Se llama DocFácil y cuesta menos que $17 al día. ¿Le interesa ver cómo funciona?

**Template C — La Referencia Social:**
> Dr. [nombre], X consultorios en Culiacán ya están usando DocFácil para su agenda y recordatorios de citas por WhatsApp. Cuesta $499/mes y la prueba es gratis. ¿Le mando info?

**Template D — La Recepcionista:**
> Buen día, ¿esta es la línea del consultorio del Dr. [nombre]? Soy [tu nombre] de DocFácil, un sistema que le facilita la vida a las recepcionistas: agenda digital con recordatorios automáticos a los pacientes. ¿Se lo podría comentar al doctor? Le puedo mandar un video cortito.

**Cadencia de seguimiento:**
- Día 1: Mensaje inicial
- Día 3: Si dejó en visto → "Dr., ¿le puedo mandar el video? Son 3 min."
- Día 7: Si no responde → "Entiendo que está ocupado. Le dejo mi WhatsApp por si más adelante necesita organizar su agenda. Que tenga buen día." (Cierre elegante, no insistente)
- Día 30: Recontactar con ángulo diferente o noticia: "Doctor, le cuento que ya sacamos [nueva función]. ¿Le interesa echarle un ojo?"

**Números:** 400 WhatsApp enviados (12 semanas) → 100 respuestas (25%) → 40 demos (40%) → 24 trials (60%) → 12 paid (50%).

---

### Canal 3: Google Maps Outreach
**Clientes esperados: (incluido en Canal 2, misma base de datos)**

Este no es un canal separado sino la FUENTE de datos para el canal 2 y el canal 1. Proceso:

1. Abrir Google Maps. Buscar "dentista Culiacán".
2. Google muestra ~200 resultados. Ir uno por uno.
3. Para cada resultado, extraer a Google Sheet:
   - Nombre del negocio
   - Nombre del doctor (si aparece)
   - Dirección
   - Teléfono
   - Rating y # de reseñas
   - ¿Tiene sitio web? (si sí, probablemente más tech-savvy)
   - Horarios (para saber cuándo visitarlos)
4. Priorizar: 3-5 estrellas con 10+ reseñas = consultorio activo y exitoso = más probable que pague.
5. Descartar: sin reseñas, cerrado permanentemente, consultorios dentro de farmacias.

**Herramienta opcional:** Si quieren automatizar la extracción, usar Outscraper.com o PhantomBuster ($20-50 USD/mes). Extrae 500+ listados en minutos.

---

### Canal 4: Programa de Referidos
**Clientes esperados: 10 de 50**

| Aspecto | Detalle |
|---|---|
| Costo | 1 mes gratis por referido ($499 de descuento) |
| Tiempo para resultados | Semana 4 en adelante |
| Dificultad | Baja una vez que tienes clientes satisfechos |
| Responsable | Ambos |

**Ejecución paso a paso:**
1. Desde el cliente #1, activar el programa. Cada cliente recibe un código único de referido.
2. Incentivo: "Por cada colega que se suscriba con tu código, te regalamos 1 mes gratis." Esto es un CAC de $499 (el mes que regalas), excelente para SaaS.
3. Pedir referidos activamente, no esperar que lleguen solos:
   - Al momento de cerrar la venta: "Doctor, ¿tiene algún colega que también le serviría?"
   - A los 14 días de uso: "¿Cómo le ha ido? ¿Conoce algún colega que ande batallando con la agenda?"
   - Al mes: "Doctor, tenemos un programa: si refiere a un colega y se suscribe, le regalamos un mes."
4. Facilitarlo: no pedir que el doctor haga nada excepto dar un nombre/WhatsApp. TÚ contactas al referido diciendo: "El Dr. [referidor] me pidió que le escribiera porque cree que DocFácil le puede servir."

**Por qué funciona:** Los dentistas en ciudades medianas se conocen entre sí. Estudiaron en la misma universidad, se ven en congresos, comparten proveedores. Un referido de un colega es casi un cierre asegurado.

**Números:** 30 clientes activos → 15 dan referidos → 25 referidos totales → 15 toman demo → 10 convierten.

---

### Canal 5: Facebook/Instagram Ads
**Clientes esperados: 5 de 50**

| Aspecto | Detalle |
|---|---|
| Costo | $3,000-$5,000 MXN total (12 semanas) |
| Tiempo para resultados | 2-3 semanas |
| Dificultad | Media (requiere saber Meta Ads básico) |
| Responsable | Fundador técnico |

**Ejecución paso a paso:**
1. NO lanzar ads antes de semana 5. Primero validar mensaje y oferta con outreach directo.
2. Campaña 1 — Leads por WhatsApp:
   - Objetivo: Mensajes (WhatsApp)
   - Presupuesto: $100 MXN/día
   - Audiencia: Sinaloa, 28-55 años, intereses: odontología, dentista, consultorio dental, ortodoncia
   - Formato: Video de 30-60 segundos mostrando la interfaz + voz explicando
   - Copy: "Recordatorios automáticos por WhatsApp a tus pacientes. Agenda digital. Cobros. Recetas. Desde $499/mes. Prueba gratis 14 días. Escríbenos."
   - CTA: Botón de WhatsApp
3. Campaña 2 — Retargeting (semana 8+):
   - Audiencia: Personas que visitaron docfacil.com en los últimos 30 días
   - Copy: "¿Ya viste DocFácil? X consultorios en Sinaloa ya lo están usando. Tu prueba gratis te espera."
   - Presupuesto: $50 MXN/día
4. Medir: Costo por mensaje de WhatsApp. Si es >$30 MXN por mensaje, pausar y ajustar creativos. Meta: $10-20 MXN por lead de WhatsApp.

**Números:** $5,000 invertidos / $15 por lead = 333 leads de WhatsApp → 80 respuestas útiles → 30 demos → 15 trials → 5-7 paid.

---

### Canal 6: Grupos de Facebook/WhatsApp de Doctores
**Clientes esperados: 3 de 50**

| Aspecto | Detalle |
|---|---|
| Costo | $0 |
| Tiempo para resultados | 3-6 semanas |
| Dificultad | Baja pero requiere paciencia |
| Responsable | Fundador técnico |

**Ejecución paso a paso:**
1. Unirse a los grupos (ver lista en semana 2).
2. Regla inquebrantable: NO VENDER DIRECTAMENTE durante las primeras 2 semanas. Solo dar valor.
3. Semana 1-2 en el grupo: Responder preguntas. Compartir tips genuinos. "Para la agenda del consultorio yo uso una herramienta que..." sin link, sin CTA.
4. Semana 3+: Publicar contenido de valor con mención natural:
   - "Hice un cálculo: un consultorio que pierde 8 citas al mes pierde ~$12,000 MXN. Nosotros creamos DocFácil exactamente para eso. ¿Alguien más tiene este problema?"
   - Responder cuando alguien pregunte "¿qué software recomiendan para consultorio?"
5. Si el admin del grupo te da permiso, publicar una oferta: "Soy cofundador de DocFácil, software para consultorios. Para miembros de este grupo: prueba gratis de 30 días (en vez de 14). Link: [...]"
6. Meta: 1-2 leads por grupo por mes. Con 5 grupos = 5-10 leads/mes = 3 clientes en 12 semanas.

---

### Canal 7: Alianzas con Distribuidores Dentales
**Clientes esperados: 3 de 50**

| Aspecto | Detalle |
|---|---|
| Costo | $200 MXN comisión por referido que se suscriba |
| Tiempo para resultados | 4-8 semanas |
| Dificultad | Media (requiere relación personal) |
| Responsable | Socio comercial |

**Ejecución paso a paso:**
1. Identificar distribuidores en Culiacán:
   - Depósitos dentales (venden insumos)
   - Distribuidores de equipo (sillones, rayos X)
   - Laboratorios dentales (coronas, prótesis)
2. Visitar en persona. Propuesta:
   - "Sus clientes son nuestros clientes. Usted ya los visita cada semana. Si nos recomienda y el dentista se suscribe, le damos $200 MXN por referido."
   - Alternativa: "Ofrecemos $300 en crédito DocFácil que usted puede regalar a sus mejores clientes como valor agregado."
3. Dejar 20 volantes + tarjetas en su mostrador.
4. Seguimiento quincenal: "¿Ha tenido oportunidad de comentar con algún doctor?"

**Números:** 5 distribuidores aliados → 2 activos → 6 referidos en 12 semanas → 3 paid.

---

### Canal 8: Eventos y Colegios de Dentistas
**Clientes esperados: 5 de 50**

| Aspecto | Detalle |
|---|---|
| Costo | $0-$500 (si hay cuota de inscripción) |
| Tiempo para resultados | 4-8 semanas |
| Dificultad | Media |
| Responsable | Ambos |

**Ejecución paso a paso:**
1. Contactar a los 3 Colegios de Dentistas de Sinaloa: Culiacán, Los Mochis, Mazatlán.
2. Propuesta: presentación gratuita de 15 min en su junta mensual. "Queremos aportar valor a sus agremiados enseñándoles a digitalizar su consultorio."
3. En la presentación:
   - NO vender 15 minutos. Dar valor 10 minutos + demo 5 minutos.
   - Ofrecer precio especial para agremiados ($499/mes en plan Básico, 6 meses gratis si entran al programa Fundador).
   - Recoger WhatsApp de TODOS los asistentes: "¿Quién quiere que le mande el link de la prueba gratis?"
4. Congresos/expos dentales: estar atento a eventos locales. Si hay un congreso dental en Mazatlán o Culiacán, asistir aunque sea como visitante. Llevar tarjetas y volantes. Platicar en los pasillos.

**Números:** 3 presentaciones × 15 asistentes promedio = 45 contactos calientes → 20 demos → 10 trials → 5 paid.

---

### Canal 9: SEO / Blog
**Clientes esperados: 2 de 50 (en estas 12 semanas; crece con el tiempo)**

| Aspecto | Detalle |
|---|---|
| Costo | $0 (tiempo del FT) |
| Tiempo para resultados | 8-16 semanas |
| Dificultad | Baja pero lenta |
| Responsable | Fundador técnico |

**Artículos a publicar (1/semana):**
1. "Software para consultorio dental México: guía completa 2026" (keyword principal)
2. "Cómo reducir citas perdidas en tu consultorio dental"
3. "Expediente clínico electrónico dental: qué necesitas saber"
4. "DocFácil vs Dentalink vs Doctocliq: comparativa 2026" (capturar tráfico de competidores)
5. "Recordatorios por WhatsApp para consultorios: cómo funciona"
6. "Cuánto cuesta un software para consultorio dental en México"
7. "Agenda digital para dentistas: por qué dejar la libreta"
8. "Software dental gratis: opciones en México 2026"

Cada artículo: 800-1,200 palabras, incluir CTA al trial, incluir FAQ con schema markup.

**Artículos por ciudad (ya tienen 18 landings):**
Asegurar que cada landing tenga al menos 300 palabras de contenido único, no solo el título cambiado.

---

### Canal 10: Google Ads
**Clientes esperados: 2-3 de 50 (si se activa)**

| Aspecto | Detalle |
|---|---|
| Costo | $2,000-$4,000 MXN total |
| Tiempo para resultados | 1-2 semanas |
| Dificultad | Media-alta |
| Responsable | Fundador técnico |

Solo activar si hay presupuesto después de semana 8 y si Facebook Ads está funcionando.

**Keywords:**
- "software consultorio dental" — CPC estimado: $8-15 MXN
- "software para dentistas" — $8-12 MXN
- "agenda dental online" — $5-10 MXN
- "recordatorio citas pacientes" — $5-8 MXN

**Configuración:**
- Solo México, solo Sinaloa (o solo las 18 ciudades con landing)
- Budget: $100/día máximo
- Landing: la página de la ciudad correspondiente
- CTA: botón de WhatsApp

---

### RESUMEN DE CANALES

| # | Canal | Clientes esperados | Costo | Tiempo |
|---|---|---|---|---|
| 1 | Visitas presenciales | 15 | $500 | 1-2 sem |
| 2 | WhatsApp frío | 12 | $0 | 1 sem |
| 3 | Referidos | 10 | ~$2,000 (meses gratis) | 4+ sem |
| 4 | Facebook/IG Ads | 5 | $3,000-$5,000 | 2-3 sem |
| 5 | Eventos/Colegios | 5 | $0-$500 | 4-8 sem |
| 6 | Grupos FB/WhatsApp | 3 | $0 | 3-6 sem |
| 7 | Distribuidores | 3 | $600 (comisiones) | 4-8 sem |
| 8 | SEO/Blog | 2 | $0 | 8-16 sem |
| 9 | Google Ads | 2-3 | $2,000-$4,000 | 1-2 sem |
| | **TOTAL** | **~57** (buffer del 15%) | **$8,000-$13,000 MXN** | |

El buffer del 15% es porque algunos canales rendirán menos de lo esperado. Tener 57 "esperados" te da margen para llegar a 50 reales.

---

## 3. EMBUDO NUMÉRICO

### Embudo General

```
CONTACTOS EN BASE DE DATOS          500
        ↓ (contactados: 80%)
CONTACTADOS (WhatsApp/visita)       400
        ↓ (responden: 25%)
RESPUESTAS / INTERÉS                100
        ↓ (aceptan demo: 50%)
DEMOS REALIZADAS                     50
        ↓ (activan trial: 60%)
TRIALS ACTIVADOS                     30
        ↓ (usan el trial: 70%)
TRIALS ACTIVOS (usaron 3+ features)  21
        ↓ (pagan: 80%)
CLIENTES PAGADOS                     ~17
```

Pero necesitamos 50, así que necesitamos 3x este embudo:

### Embudo para 50 Clientes

```
CONTACTOS EN BASE DE DATOS        1,500
        ↓ (contactados: 80%)
CONTACTADOS                        1,200
        ↓ (responden: 25%)
RESPUESTAS / INTERÉS                 300
        ↓ (aceptan demo: 50%)
DEMOS REALIZADAS                     150
        ↓ (activan trial: 60%)
TRIALS ACTIVADOS                      90
        ↓ (pagan: 55%)
CLIENTES PAGADOS                      50
```

**Nota:** La tasa trial→paid del 55% es ambiciosa pero alcanzable si el onboarding es excelente. Si baja a 40%, necesitas 125 trials = 200 demos = 400 respuestas.

### Embudo por Canal

| Canal | Contactos | Respuestas | Demos | Trials | Paid | Conv. total |
|---|---|---|---|---|---|---|
| Visitas presenciales | 200 | 80 (40%) | 35 (44%) | 25 (71%) | 15 (60%) | 7.5% |
| WhatsApp frío | 400 | 100 (25%) | 40 (40%) | 24 (60%) | 12 (50%) | 3.0% |
| Referidos | 60 | 45 (75%) | 30 (67%) | 20 (67%) | 10 (50%) | 16.7% |
| Facebook Ads | 333 | 80 (24%) | 30 (38%) | 15 (50%) | 5 (33%) | 1.5% |
| Eventos/Colegios | 45 | 35 (78%) | 20 (57%) | 12 (60%) | 5 (42%) | 11.1% |
| Grupos FB | 100 | 30 (30%) | 12 (40%) | 8 (67%) | 3 (38%) | 3.0% |
| Distribuidores | 30 | 15 (50%) | 8 (53%) | 5 (63%) | 3 (60%) | 10.0% |
| SEO/Blog | 200 (visitas) | 20 (10%) | 8 (40%) | 5 (63%) | 2 (40%) | 1.0% |
| Google Ads | 100 (clics) | 25 (25%) | 10 (40%) | 6 (60%) | 3 (50%) | 3.0% |
| **TOTALES** | **~1,468** | **~430** | **~193** | **~120** | **~58** | **3.9%** |

Los 58 esperados te dan un buffer cómodo para llegar a 50.

### Velocidad Requerida por Semana

| Semana | Contactos nuevos/sem | Demos/sem | Trials/sem | Paid acumulado |
|---|---|---|---|---|
| 1 | 180 | 3 | 0 | 0 |
| 2 | 100 | 5 | 3 | 1 |
| 3 | 70 | 5 | 3 | 3 |
| 4 | 50 | 5 | 4 | 6 |
| 5 | 80 | 8 | 5 | 11 |
| 6 | 80 | 6 | 4 | 16 |
| 7 | 60 | 8 | 5 | 22 |
| 8 | 50 | 7 | 4 | 27 |
| 9 | 50 | 6 | 4 | 33 |
| 10 | 40 | 6 | 4 | 38 |
| 11 | 30 | 10 | 5 | 44 |
| 12 | 30 | 6 | 4 | **50** |
| **Total** | **~900** | **~75** | **~45** | **50** |

**Nota:** Los "paid acumulado" incluyen clientes que entraron como trial semanas antes. No todo cierre es inmediato; hay un lag de 1-3 semanas entre trial y pago.

---

## 4. PRICING STRATEGY PARA LOS PRIMEROS 50

### Estrategia: "Precio Fundador" (NO regalar todo)

**Principio:** No regales el producto. Un doctor que no paga no valora. Un doctor que paga $499/mes se comporta muy diferente a uno que usa algo gratis. El compromiso financiero activa el compromiso de uso.

### Oferta para los Primeros 50 Clientes

**Dos niveles distintos — no confundirlos:**

- **Primeros 10 → programa Fundador.** Es el beneficio caro: 6 meses gratis y precio congelado de por vida, con onboarding 1:1 de Omar. Es el número real de `config/founders.php`, y es chico a propósito: el compromiso es permanente y la atención la da una sola persona.
- **Los siguientes 40 → clientes normales.** Pagan desde su primer mes. Conservan los beneficios de abajo (trial, migración, onboarding), pero **sin** el congelamiento de precio.

| Concepto | Oferta |
|---|---|
| **Trial** | 14 días gratis, sin tarjeta (reduce fricción al máximo) |
| **Precio Fundador (Básico)** | 6 meses gratis y después $499/mes congelado de por vida — solo para los primeros 10 (así está definido en `config/founders.php`) |
| **Precio Fundador (Pro)** | $999/mes si sube a Pro (el precio fundador congela el plan contratado, no el catálogo) |
| **Precio Fundador (Clínica)** | $1,999/mes si sube a Clínica (mismo criterio) |
| **Protección de precio** | "Este precio se te queda PARA SIEMPRE mientras mantengas tu suscripción activa: si el catálogo sube, tu mensualidad no sube" |
| **Migración gratis** | "Te cargamos tus primeros 50 pacientes al sistema sin costo" |
| **Onboarding personalizado** | Llamada 1-a-1 de 30 min para configurar todo |

### Por Qué Funciona

1. **6 meses gratis, no "gratis para siempre":** el doctor entra sin riesgo, pero ya sabe que a partir del mes 7 paga $499/mes. El que nunca paga no valora; el que se compromete usa el sistema en serio desde el día 1.
2. **"Para siempre":** Elimina el miedo a que suba el precio. El doctor siente que está ganando algo exclusivo: su mensualidad queda congelada de por vida.
3. **"Primeros 10":** Crea urgencia real. "Ya van 7 consultorios con precio fundador. Quedan 3 lugares." No es escasez de teatro: son los lugares reales del programa, contados contra la base de fundadores.
4. **Migración gratis:** El mayor miedo del doctor es "voy a tener que meter TODO desde cero". Quítalo.
5. **El trato es explícito:** 6 meses gratis a cambio de retroalimentación y testimonial. El doctor entiende qué recibe y qué da a cambio.

### Estrategia de Upgrade: Free → Básico → Pro

**Free a Básico ($499/mes):**
- El plan Free tiene 30 pacientes y 20 citas/mes. Un consultorio activo llega a ese límite en 2-3 semanas.
- Cuando el doctor llegue al límite, mostrar mensaje: "Llegaste al límite del plan gratuito. Actualiza a Básico por $499/mes para pacientes ilimitados. [Actualizar ahora]"
- NUNCA bloquear funciones críticas (no borrar datos, no impedir ver pacientes existentes). Solo bloquear CREAR nuevos.
- WhatsApp automático al fundador cuando un usuario Free llegue al 80% de su límite: para que llame proactivamente.

**Básico a Pro ($999/mes):**
- Trigger: cuando el consultorio agrega un segundo doctor o necesita reportes avanzados.
- No empujar el upgrade activamente durante los primeros 3 meses. Primero que adopten, después que crezcan.
- A los 3 meses, enviar: "Doctor, vi que ya tiene [X pacientes]. ¿Sabía que con el plan Pro tiene reportes de ingresos y puede agregar más doctores? Le cuento cómo funciona."

### Qué NO Hacer

- **NO dar trials de 30 días.** 14 días es suficiente. Trials largos = menos urgencia = menor conversión.
- **NO regalar meses gratis** a prospectos que no han probado (regálalos solo como incentivo de referidos a clientes existentes).
- **NO hacer descuentos adicionales** sobre el precio fundador. Si el doctor regatea, la respuesta es: "El programa fundador ya incluye 6 meses gratis y tu precio queda congelado de por vida. Es el mejor trato que vamos a tener."
- **NO prometer un descuento que no existe.** El precio de lista del Básico es $499/mes, igual que el del fundador: lo que distingue al fundador son los 6 meses gratis y la garantía de precio, no una mensualidad más baja. Nunca decir "precio fundador con descuento".
- **NO cobrar anualidad** todavía. Mensual reduce la barrera de entrada. Puedes ofrecer anual (con 2 meses gratis) después del cliente #100.

### Proyección de MRR con 50 Clientes

Con el catálogo vigente: Básico $499, Pro $999, Clínica $1,999.

| Plan | % distribución | # clientes | Precio mensual | MRR |
|---|---|---|---|---|
| Free | 0% (no cuenta) | 0 | $0 | $0 |
| Básico | 60% | 30 | $499 | $14,970 |
| Pro | 30% | 15 | $999 | $14,985 |
| Clínica | 10% | 5 | $1,999 | $9,995 |
| **Total** | | **50** | | **$39,950 MXN/mes** |

Solo los **10 fundadores** tienen 6 meses gratis; los otros **40 pagan desde su primer mes**. Por eso el MRR cobrado en la semana 12 es de **~$31,960 MXN/mes** (40 × $799 de ARPU), no $0. A partir del mes 7, cuando los 10 fundadores empiezan a pagar, los 50 clientes suman **$39,950 MXN/mes** con la mezcla 60/30/10 de arriba.

---

## 5. HERRAMIENTAS Y PROCESOS

### 5.1 CRM Workflow (Panel de Ventas Existente)

**Estados del prospecto en el pipeline:**

```
NUEVO → CONTACTADO → RESPONDIÓ → DEMO_AGENDADA → DEMO_REALIZADA → TRIAL → ACTIVO_PAID → CHURNED
                                                                      ↘ NO_INTERESADO
                                                         ↘ TRIAL_EXPIRADO (→ recontactar en 30 días)
```

**Reglas de movimiento:**
- NUEVO → CONTACTADO: se envió primer WhatsApp o se visitó presencialmente
- CONTACTADO → RESPONDIÓ: el doctor o recepcionista contestó (aunque sea "no me interesa")
- RESPONDIÓ → DEMO_AGENDADA: hay fecha y hora confirmada
- DEMO_AGENDADA → DEMO_REALIZADA: se hizo la demo (registrar: duración, features que le interesaron, objeciones, siguiente paso)
- DEMO_REALIZADA → TRIAL: el doctor activó su cuenta de prueba
- TRIAL → ACTIVO_PAID: el doctor pagó su primer mes
- TRIAL → TRIAL_EXPIRADO: pasaron 14 días sin pago (agregar a lista de recontacto)

**Campos obligatorios por prospecto:**
- Nombre del doctor
- Nombre del consultorio
- Ciudad
- Teléfono/WhatsApp
- Canal de origen (visita / whatsapp_frio / referido / facebook_ad / evento / seo / google_ad / grupo_fb / distribuidor)
- Fecha de primer contacto
- Fecha de último contacto
- Notas (nombre de recepcionista, # sillones, objeciones escuchadas)
- Siguiente acción (fecha + qué hacer)

### 5.2 Métricas a Trackear

**DIARIO (revisar cada noche, 5 minutos):**
- WhatsApp enviados hoy
- Respuestas recibidas hoy
- Demos agendadas hoy
- Trials activados hoy

**SEMANAL (revisar viernes PM, 30 minutos):**
| Métrica | Meta semanal |
|---|---|
| Contactos nuevos en pipeline | 50+ |
| Demos realizadas | 6+ |
| Trials activados | 4+ |
| Clientes pagados nuevos | 4+ (promedio para llegar a 50 en 12 sem) |
| Tasa respuesta WhatsApp | >20% |
| Tasa demo → trial | >55% |
| Tasa trial → paid | >50% |
| Churn (cancelaciones) | 0 |

**MENSUAL:**
- MRR total y crecimiento
- CAC por canal (costo total del canal / clientes obtenidos)
- LTV estimado (asumiendo 12 meses de retención promedio)
- NPS de clientes activos
- Features más pedidas por clientes

### 5.3 Proceso de Onboarding del Cliente (Primeros 7 Días)

**Día 0 (momento del registro):**
- Automático: WhatsApp de bienvenida: "¡Bienvenido a DocFácil, Dr. [nombre]! Soy [nombre fundador]. En los próximos 10 minutos voy a ayudarle a configurar su consultorio. ¿Está disponible?"
- Si responde: llamada/videollamada de 15 min. Configurar juntos: nombre del consultorio, horarios, primer paciente, primera cita.
- Si no responde: enviar video tutorial de 5 min: "Cómo configurar DocFácil en 10 minutos".

**Día 1:**
- WhatsApp: "Dr. [nombre], ¿ya agendó su primera cita en DocFácil? Si quiere, yo le ayudo a cargar sus primeros 10 pacientes. Solo mándeme la lista por WhatsApp (nombre + teléfono) y yo los doy de alta."
- Meta: que tenga al menos 5 pacientes y 3 citas en el sistema.

**Día 3:**
- WhatsApp: "¿Cómo le fue con los recordatorios de citas? Sus pacientes deberían haber recibido un WhatsApp automático ayer. ¿Todo bien?"
- Meta: que haya visto un recordatorio salir exitosamente.

**Día 5:**
- WhatsApp: "Doctor, un tip: puede generar recetas PDF directamente desde la consulta. Le ahorra 5 minutos por paciente. ¿Quiere que le muestre cómo?"
- Meta: activar una segunda feature (recetas o cobros).

**Día 7:**
- WhatsApp: "¡Una semana con DocFácil! ¿Cómo va? ¿Hay algo que quisiera que hiciera diferente? Su opinión nos importa mucho."
- Meta: obtener feedback + detectar problemas antes de que decida no renovar.

**Día 10:**
- WhatsApp: "Doctor, su prueba gratis termina en 4 días. ¿Quiere continuar con el plan Básico a $499/mes? Le mando el link de pago."
- Si no responde en 24 hrs: LLAMAR por teléfono.

**Día 14 (fin de trial):**
- Si pagó: "¡Gracias por confiar en DocFácil! Ahora sí es oficial. ¿Tiene algún colega al que le pueda servir? Si se suscribe, le regalamos un mes gratis."
- Si no pagó: "Doctor, su prueba terminó pero sus datos siguen guardados 30 días. Si cambia de opinión, aquí estamos." (No borrar datos inmediatamente, genera mala voluntad.)

### 5.4 Proceso de Activación ("Aha Moment")

El "Aha moment" de DocFácil es cuando el doctor ve que un paciente confirmó su cita por WhatsApp automáticamente. Ese es el momento en que dice "esto sí funciona."

**Checklist de activación (hacer en los primeros 3 días):**
1. [ ] Consultorio configurado (nombre, dirección, horarios)
2. [ ] Al menos 5 pacientes registrados con teléfono
3. [ ] Al menos 3 citas agendadas para los próximos 7 días
4. [ ] Recordatorio enviado y recibido por al menos 1 paciente
5. [ ] El doctor vio la confirmación del paciente en el sistema

**Si el trial no completa el checklist en 3 días:** llamar. No esperar. Un trial inactivo NUNCA se convierte en cliente pagado.

**Métrica clave de activación:** % de trials que completan los 5 pasos del checklist en 72 horas. Meta: >70%.

---

## 6. RIESGOS Y CONTINGENCIAS

### Riesgo 1: No hay tracción en semana 4 (<4 clientes pagados)

**Diagnóstico:**
- ¿Tienes suficientes demos? Si <10 demos en 4 semanas, el problema es TOP OF FUNNEL (no estás llegando a suficientes doctores).
- ¿Tienes demos pero no trials? El problema es la DEMO (el pitch no conecta, estás mostrando features equivocadas).
- ¿Tienes trials pero no pagan? El problema es ACTIVACIÓN/PRICING (el doctor no encuentra valor o el precio es alto).

**Plan de acción por escenario:**

**Si el problema es top-of-funnel:**
- Duplicar visitas presenciales. De 25/semana a 50/semana. El SC dedica 100% de su tiempo a esto.
- Contratar a un "promotor" temporal: un joven con moto que visite consultorios y deje volantes + tome fotos del consultorio para enriquecer la base. Costo: $2,000-$3,000/semana.
- Activar Facebook Ads inmediatamente (no esperar a semana 5).

**Si el problema es la demo:**
- Grabar las demos (con permiso) y revisarlas. ¿Estás hablando de features o de problemas del doctor?
- Cambiar el formato: en vez de "te muestro el sistema", preguntar primero: "Doctor, ¿cuál es su mayor dolor de cabeza con la agenda/cobros?" y luego mostrar SOLO la solución a ese dolor.
- Invitar a un doctor ya cliente a una demo conjunta: "Le presento al Dr. Pérez, él usa DocFácil desde hace 3 semanas. ¿Doctor, les platica cómo le ha ido?"

**Si el problema es trial → paid:**
- Llamar a los 3 trials más recientes que no convirtieron. Preguntar directamente: "¿Qué le faltó? ¿Por qué no continuó?"
- Si dicen "no tuve tiempo de probarlo": ofrecer onboarding donde TÚ configuras todo. "Yo le doy de alta sus pacientes, le configuro la agenda y usted solo llega a usarlo."
- Si dicen "está caro": bajar temporalmente a $299/mes para validar si el precio es realmente el freno (si aún no compran a $299, el problema NO es el precio).
- Si dicen "le falta X feature": evaluar si es una feature que puedes hacer en 1-2 días. Si sí, hazla y recontacta: "Doctor, ya tenemos [lo que pidió]."

### Riesgo 2: Visitas presenciales no funcionan

**Señales:** <5% de las visitas resultan en demo agendada después de 2 semanas completas.

**Plan B:**
1. Cambiar horario: probar visitas de 6:30-7:30 PM cuando los doctores están cerrando y más relajados.
2. Cambiar al actor: si el SC no está generando confianza, que el FT (el que construyó el sistema) haga las visitas. "Yo soy el que lo creó" genera más credibilidad.
3. Cambiar el hook: en vez de "vengo a venderle software", probar "vengo a regalarle un sistema de recordatorios por WhatsApp para su consultorio. Es gratis los primeros 14 días."
4. Pivotar a llamadas telefónicas: si las visitas no funcionan, invertir ese tiempo en llamadas. 30 llamadas/día > 10 visitas/día en alcance.
5. Pivotar a un "micro-evento": organizar un desayuno gratuito para 10 dentistas en un restaurante de Culiacán. Costo: $2,000-$3,000 MXN. "Los invito a desayunar para platicarles cómo otros consultorios están reduciendo sus citas perdidas." 15 min de presentación, resto networking.

### Riesgo 3: El ICP está mal

**Señales:** Los dentistas consistentemente dicen "no necesito software" O "ya tengo uno y estoy contento" O "no tengo presupuesto para eso".

**Plan de pivote:**

**Pivote A — Cambiar de especialidad dental a médica:**
- Probar con médicos generales, pediatras, ginecólogos, dermatólogos. Son el mismo producto, diferente mercado.
- Ventaja: hay 3x más médicos generales que dentistas en Culiacán.
- Acción: crear landing docfacil.com/medicos, ajustar mensajes ("expediente clínico" en vez de "odontograma"), probar WhatsApp frío con 50 médicos.

**Pivote B — Cambiar de ciudad:**
- Si Culiacán no responde, probar Mazatlán (turismo = dentistas cosméticos con más presupuesto) o Guadalajara (más grande, más early adopters).
- Mazatlán tiene la ventaja de que está a 3 hrs y puedes ir presencialmente.

**Pivote C — Cambiar el modelo de venta:**
- De "SaaS mensual" a "setup + mensualidad": cobrar $2,000-$3,000 de instalación + $499/mes. Algunos doctores prefieren pagar más upfront y menos mensual.
- De "self-service trial" a "done-for-you setup": el doctor no toca nada, tú configuras todo y le entregas su consultorio digital funcionando.

**Pivote D — Cambiar el feature principal:**
- Si "agenda y recordatorios" no emocionan, probar con "recetas digitales" como gancho (muchos doctores odian hacer recetas a mano).
- O probar con "cobros y reportes financieros" (el ángulo de "¿sabes cuánto ganas realmente al mes?").

### Riesgo 4: Churn alto (>10% mensual)

**Señales:** 3+ clientes cancelan en el primer mes.

**Acción inmediata:**
1. Llamar a CADA churned customer. "Doctor, lamentamos que se fue. ¿Qué pasó? ¿Qué podríamos haber hecho diferente?"
2. Clasificar razones: ¿precio? ¿no lo usaban? ¿falta feature? ¿bug?
3. Si es precio: re-evaluar si $499 es demasiado para el segmento. Considerar $199/mes como plan entry-level.
4. Si es adopción: el onboarding está fallando. Agregar más touchpoints humanos (llamada día 1, día 3, día 7).
5. Si es falta de feature: priorizar desarrollo de esa feature.

### Riesgo 5: El socio comercial no puede dedicar tiempo completo

**Plan:** Contratar un vendedor comisionista:
- Sueldo base: $0 o $3,000-$5,000/mes
- Comisión: $500 por cliente que cierre y pague primer mes + $100 recurrente mensual mientras el cliente siga activo
- Perfil: joven (22-28), con experiencia en ventas B2B, que tenga moto o carro, que conozca Culiacán
- Dónde encontrarlo: OCC, Indeed, grupos de Facebook de empleos en Culiacán
- Ya tienen sistema de comisiones construido en DocFácil, así que es plug-and-play.

---

## APÉNDICE A: CALENDARIO VISUAL DE 12 SEMANAS

```
SEM 1  ████████░░░░ Pipeline build: 180 contactos, 25 visitas, materiales listos
SEM 2  █████████░░░ WhatsApp masivo: 100 enviados, 5 demos, 3 trials
SEM 3  ██████████░░ Segundo round: Los Mochis, distribuidores, 6 trials acumulados
SEM 4  ██████████░░ Cierre tibios: 5-7 paid, CHECKPOINT, referidos inician
SEM 5  ███████████░ Escalar canal ganador: 10-12 paid, caso de éxito
SEM 6  ███████████░ Mazatlán prospecting, Facebook Ads inician: 15-17 paid
SEM 7  ███████████░ Evento Colegio: 20-23 paid
SEM 8  ████████████ MITAD: 25-28 paid, Mazatlán visitas, ajustar funnel
SEM 9  ████████████ Referidos como motor: 30-33 paid
SEM 10 ████████████ Médicos no-dentales, segundo Colegio: 35-38 paid
SEM 11 ████████████ Sprint de cierre: 42-46 paid
SEM 12 ████████████ META: 50 CLIENTES PAGADOS 🎯 MRR cobrado: ~$31,960 (40 pagando; los 10 fundadores en sus 6 meses gratis); MRR al mes 18: ~$39,950
```

## APÉNDICE B: PRESUPUESTO TOTAL ESTIMADO (12 SEMANAS)

| Concepto | Costo |
|---|---|
| Volantes e impresiones (500 volantes + 200 tarjetas) | $800 |
| Gasolina para visitas (Culiacán) | $3,000 |
| Viajes a Los Mochis (2 viajes) | $1,500 |
| Viaje a Mazatlán (1 viaje) | $2,000 |
| Facebook/Instagram Ads | $4,000 |
| Google Ads (opcional) | $2,000 |
| Desayuno/evento para dentistas (1 evento) | $3,000 |
| Comisiones a distribuidores | $600 |
| Meses gratis a clientes por referidos | $2,000 |
| Herramientas (Outscraper, Canva Pro) | $500 |
| **TOTAL** | **$19,400 MXN (~$1,100 USD)** |

**Costo de adquisición promedio (CAC):** $19,400 / 50 = **$388 MXN por cliente** (~$22 USD).

Con un ARPU de ~$799 MXN/mes (mezcla 60/30/10: 0.60 × $499 + 0.30 × $999 + 0.10 × $1,999 = $799), un promedio de 10.8 meses pagados dentro de los primeros 12 (los 10 fundadores tienen 6 meses gratis; los otros 40 pagan desde el mes 1) y retención estimada de 12+ meses, el LTV del primer año es 10.8 × $799 = ~$8,629 MXN.

**Ratio LTV:CAC = $8,629 / $388 = 22.2:1** (excelente; todo arriba de 3:1 es sano para SaaS).

## APÉNDICE C: DECISIONES CLAVE SEMANA POR SEMANA

| Semana | Decisión |
|---|---|
| 4 | ¿Seguimos con el canal actual o pivotamos? |
| 6 | ¿Activamos Facebook Ads o no hay presupuesto? |
| 7 | ¿La presentación en Colegios funcionó? ¿Repetimos? |
| 8 | ¿Contratamos vendedor comisionista? |
| 10 | ¿Expandimos a médicos no-dentales? |
| 12 | ¿Subimos precios para los próximos 50? ¿Contratamos? |

---

*Plan creado para DocFácil | Abril 2026 | Revisión sugerida: cada 4 semanas*
