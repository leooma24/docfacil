    ...
    <div class="rounded-2xl p-4 md:p-6 text-white" style="background:linear-gradient(135deg,#0d9488,#0891b2);">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3 md:gap-4">
                <div class="w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center shrink-0" style="background:rgba(255,255,255,0.2);">
                    <span class="text-base md:text-xl font-extrabold">{{ substr($next->patient->first_name, 0, 1) }}{{ substr($next->patient->last_name, 0, 1) }}</span>
                </div>
                <div class="min-w-0">
                    <div class="text-xs uppercase tracking-wide font-semibold" style="opacity:0.8;font-size:0.65rem;">Siguiente paciente</div>
                    <div class="text-lg md:text-xl font-extrabold truncate">{{ $next->patient->first_name }} {{ $next->patient->last_name }}</div>
                    <div class="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs md:text-sm mt-0.5" style="opacity:0.9;">
