    ...
    @if($datos['contestaron']->isNotEmpty())
        <div style="margin-bottom:1.75rem;">
            ...
            @foreach($datos['contestaron'] as $p)
                @php $paso = $this->siguientePaso($p); @endphp
                <div style="display:flex;align-items:center;gap:0.75rem;background:#fffbeb;border:1px solid #fde68a;border-radius:0.75rem;padding:0.85rem 1rem;margin-bottom:0.5rem;">
                    <div style="flex:1;min-width:0;">
                        ...
                        {{-- Una sola acción, la que toca. Quien intenta vender,
                             agendar y pedir referido en el mismo mensaje no
                             consigue ninguna de las tres. --}}
                        <div style="font-size:0.78rem;color:#92400e;margin-top:3px;">
                            <strong>{{ $paso['que'] }}</strong> — {{ $paso['porque'] }}
                        </div>

                        {{-- El consejo, pegado a la acción. Se ve hasta que
                             sale natural y entonces se apaga. --}}
                        @php $tip = $this->tipDe($paso['etapa']); @endphp
                        @if($tip)
                            <div style="display:flex;align-items:flex-start;gap:6px;font-size:0.75rem;color:#4b5563;background:#fff;border:1px dashed #d1d5db;border-radius:0.5rem;padding:6px 8px;margin-top:5px;">
                                <span>
                                    @if($tip['es_repaso'] ?? false)<span style="color:#9ca3af;">Repaso ·</span>@endif
                                    <strong>{{ $tip['tip'] }}</strong> {{ $tip['porque'] }}
                                </span>
                                <button type="button" wire:click="yaMeSaleSolo('{{ $tip['clave'] }}')" title="Dejar de verlo"
                                        style="flex:none;margin-left:auto;color:#9ca3af;font-size:0.7rem;white-space:nowrap;">ya me sale solo</button>
                            </div>
                        @endif
                    </div>
                    <a href="{{ $this->ligaWhatsApp($p) }}" target="_blank" rel="noopener" wire:click="registrarEnvio({{ $p->id }})"
                       style="flex:none;background:#fff;border:1px solid #d1d5db;color:#374151;font-weight:600;font-size:0.82rem;padding:0.5rem 0.9rem;border-radius:0.6rem;text-decoration:none;">Ver chat</a>

                    @if($paso['mensaje'])
                        <a href="{{ $paso['mensaje'] }}" target="_blank" rel="noopener" wire:click="registrarEnvio({{ $p->id }})"
                           style="flex:none;background:#25d366;color:#05330f;font-weight:700;font-size:0.82rem;padding:0.5rem 0.9rem;border-radius:0.6rem;text-decoration:none;">{{ $paso['que'] }}</a>
                    @elseif($paso['boton'] === 'demo_hecha')
                        <button type="button" wire:click="marcarDemoHecha({{ $p->id }})"
                                style="flex:none;background:#0f766e;color:#fff;font-weight:700;font-size:0.82rem;padding:0.5rem 0.9rem;border-radius:0.6rem;">Sí la hice</button>
                    @endif

                    <button type="button" wire:click="abrirReporte({{ $p->id }})" title="Reportar algo"
                ...
                @if($reportando === $p->id)
                    <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:0.75rem;padding:0.85rem 1rem;margin:-0.2rem 0 0.6rem;">
                        ...
                        <textarea wire:model="reporte" rows="2"
