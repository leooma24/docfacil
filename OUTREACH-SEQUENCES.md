# DocFacil - Secuencias de Outreach para Vendedores

> **Objetivo:** Cerrar los primeros 50 clientes de pago.
> **Mercado:** Consultorios medicos y dentales en Culiacan, Sinaloa (y expansion nacional).
> **Planes:** Gratis $0 | Basico $149/mes | Profesional $299/mes | Clinica $499/mes

---

## Variables para personalizar

| Variable | Ejemplo |
|---|---|
| `[NOMBRE_DOCTOR]` | Dr. Martinez |
| `[NOMBRE_VENDEDOR]` | Omar |
| `[ESPECIALIDAD]` | odontologia / medicina general / cardiologia |
| `[NOMBRE_CONSULTORIO]` | Dental Express |
| `[ZONA]` | Col. Centro, Culiacan |
| `[LINK_DEMO]` | https://docfacil.com/demo |
| `[LINK_TRIAL]` | https://docfacil.com/registro |
| `[TELEFONO_VENDEDOR]` | 668-XXX-XXXX |
| `[PLAN_RECOMENDADO]` | Basico / Profesional / Clinica |
| `[PRECIO]` | $149 / $299 / $499 |
| `[DESCUENTO]` | 30% los primeros 3 meses |

---

# SECUENCIA 1: WhatsApp Frio (Primer Contacto)

## Variante A: Dentistas

### Mensaje 1 - Dia 0 (Primer contacto)
**Mejor horario:** Martes a jueves, 1:00-2:30 PM (hora de comida entre pacientes)

```
Hola [NOMBRE_DOCTOR], buenas tardes. Soy [NOMBRE_VENDEDOR] de DocFacil.

Vi que tiene su consultorio dental en [ZONA] y queria preguntarle algo rapido:

Como lleva el control de sus citas y expedientes de pacientes? Todavia en libreta/Excel o ya usa algun sistema?

Le pregunto porque estamos trabajando con varios dentistas aqui en Culiacan y el tema mas comun es que pierden citas porque los pacientes se les olvida, y eso son miles de pesos al mes que se van.

DocFacil manda recordatorios automaticos por WhatsApp a sus pacientes y le organiza toda la agenda. Se lo puedo mostrar en 10 minutos si le interesa.

Que dia le queda mejor esta semana?
```

### Mensaje 2 - Dia 2 (Si no responde)
**Mejor horario:** Miercoles o jueves, 10:00-11:00 AM

```
Hola [NOMBRE_DOCTOR], le escribo de nuevo rapido.

Le comparto un dato que nos dicen los dentistas que ya usan DocFacil: en promedio recuperan entre 8 y 12 citas al mes nada mas con los recordatorios automaticos por WhatsApp.

Si cada cita suya vale $500-$800, estamos hablando de $4,000 a $9,000 pesos mas al mes. Y el sistema cuesta $149.

Le dejo este video de 2 minutos donde se ve como funciona: [LINK_DEMO]

Si tiene 10 minutos esta semana le hago la demo en vivo. Sin compromiso.
```

### Mensaje 3 - Dia 5 (Ultimo intento)
**Mejor horario:** Viernes, 12:00-1:00 PM

```
[NOMBRE_DOCTOR], ultimo mensaje, no quiero ser molesto.

Solo queria dejarle esto: puede probar DocFacil gratis, sin meter tarjeta ni nada. Si le sirve, perfecto. Si no, no pasa nada.

[LINK_TRIAL]

Si en algun momento quiere platicarlo, aqui estoy. Que tenga excelente fin de semana.

- [NOMBRE_VENDEDOR]
[TELEFONO_VENDEDOR]
```

---

## Variante B: Medicos Generales

### Mensaje 1 - Dia 0
**Mejor horario:** Martes a jueves, 1:00-2:30 PM

```
Hola [NOMBRE_DOCTOR], buenas tardes. Soy [NOMBRE_VENDEDOR] de DocFacil.

Tengo entendido que tiene su consultorio en [ZONA]. Le quiero hacer una pregunta rapida y honesta:

Cuantas veces a la semana le pasa que un paciente no llega a su cita sin avisar?

Es el problema #1 que nos dicen los medicos generales. Por eso hicimos DocFacil: un sistema que le manda recordatorios por WhatsApp a sus pacientes un dia antes, automatico. Usted no tiene que hacer nada.

Ademas le organiza expedientes, citas, cobros y hasta genera recetas PDF con sus datos.

Le puedo mostrar como funciona en 10 minutos. Que dia le queda mejor?
```

### Mensaje 2 - Dia 2
**Mejor horario:** Miercoles o jueves, 10:00-11:00 AM

```
Hola [NOMBRE_DOCTOR], le doy seguimiento rapido.

Un doctor aqui en Culiacan nos conto que antes de DocFacil tenia 5-6 pacientes a la semana que no llegaban. Con los recordatorios automaticos, bajo a 1-2.

Haga la cuenta: 4 pacientes recuperados x $400-$600 la consulta = $1,600-$2,400 mas a la semana. El sistema cuesta $149 al mes.

Si quiere verlo funcionando: [LINK_DEMO]

10 minutos y le muestro todo. Que dia le acomoda?
```

### Mensaje 3 - Dia 5
**Mejor horario:** Viernes, 12:00-1:00 PM

```
[NOMBRE_DOCTOR], no le quito mas tiempo. Solo le dejo el acceso gratuito por si quiere probarlo por su cuenta:

[LINK_TRIAL]

Es gratis, sin tarjeta, y lo puede usar desde su celular. Si necesita ayuda, aqui estoy.

Que tenga buen fin de semana.

- [NOMBRE_VENDEDOR]
[TELEFONO_VENDEDOR]
```

---

## Variante C: Especialistas

### Mensaje 1 - Dia 0
**Mejor horario:** Martes a jueves, 1:00-2:30 PM

```
Hola [NOMBRE_DOCTOR], buenas tardes. Soy [NOMBRE_VENDEDOR] de DocFacil.

Le escribo porque estamos trabajando con especialistas en [ESPECIALIDAD] aqui en la region y hay un tema que sale siempre: el tiempo que se pierde llenando expedientes, buscando historiales y organizando la agenda.

DocFacil es un sistema disenado especificamente para consultorios medicos. Le permite:

- Tener todos los expedientes digitales y organizados
- Agenda inteligente con recordatorios por WhatsApp
- Recetas PDF profesionales con su cedula y datos
- Reportes de ingresos y pacientes

Todo desde su computadora o celular. Le puedo hacer una demo personalizada de 15 minutos para que vea como aplicaria en [ESPECIALIDAD].

Tiene espacio esta semana?
```

### Mensaje 2 - Dia 2
**Mejor horario:** Miercoles o jueves, 10:00-11:00 AM

```
Hola [NOMBRE_DOCTOR], le doy seguimiento.

Algo que nos dicen mucho los especialistas es que necesitan un expediente clinico mas completo que un medico general: diagnosticos detallados, tratamientos en curso, fotos, estudios...

DocFacil le permite guardar todo eso por paciente, y lo tiene disponible en 2 clics cuando el paciente llega a consulta. Ya no tiene que buscar en carpetas o archivos.

Le dejo una demo rapida aqui: [LINK_DEMO]

Y si prefiere que se la muestre en vivo, con gusto. Que dia le funciona?
```

### Mensaje 3 - Dia 5
**Mejor horario:** Viernes, 12:00-1:00 PM

```
[NOMBRE_DOCTOR], ultimo mensaje de mi parte.

Puede crear su cuenta gratis y probar DocFacil sin compromiso: [LINK_TRIAL]

El plan gratuito incluye hasta 30 pacientes para que vea si le funciona. Si le convence, platicamos sobre el plan que mejor se ajuste a su consultorio.

Estoy a sus ordenes.

- [NOMBRE_VENDEDOR]
[TELEFONO_VENDEDOR]
```

---

# SECUENCIA 2: Post-Visita Presencial

> **Contexto:** El vendedor visito el consultorio en persona. Puede que haya hablado con el doctor o con la recepcionista.

### Mensaje 1 - Mismo dia (dentro de 2-3 horas despues de la visita)
**Mejor horario:** Enviar entre 5:00-7:00 PM del mismo dia

```
Hola [NOMBRE_DOCTOR], soy [NOMBRE_VENDEDOR], el que paso hoy por su consultorio [NOMBRE_CONSULTORIO].

Gracias por su tiempo. Como le comente, DocFacil le puede ayudar a:

1. Que ningun paciente se le olvide su cita (recordatorios WhatsApp automaticos)
2. Tener todos sus expedientes digitales y ordenados
3. Generar recetas profesionales en PDF en segundos

Le dejo el link para que le eche un ojo cuando tenga chance: [LINK_DEMO]

Y si quiere, le hago la demo en vivo por videollamada o paso de nuevo al consultorio. Lo que le quede mas comodo.

Quedo al pendiente!
```

### Mensaje 2 - Dia 2 (Caso de uso relevante)
**Mejor horario:** Martes a jueves, 10:00-11:00 AM

```
[NOMBRE_DOCTOR], buenos dias.

Le platico algo rapido: un [dentista/medico] aqui en Culiacan empezo a usar DocFacil hace 2 meses. Lo que mas le gusto fue lo de los recordatorios.

Antes perdia como 6-8 citas a la semana porque los pacientes no llegaban. Ahora les llega un WhatsApp automatico un dia antes y sus faltas bajaron a 1-2 por semana.

El calcula que eso le representa unos $8,000-$10,000 pesos mas al mes. Y paga $149 por el sistema.

Si quiere lo prueba gratis y ve los resultados usted mismo: [LINK_TRIAL]

O si prefiere, le hago la demo rapido cualquier dia de esta semana. Que le parece?
```

### Mensaje 3 - Dia 5 (Oferta fundador)
**Mejor horario:** Miercoles o jueves, 1:00-2:00 PM

```
[NOMBRE_DOCTOR], tengo algo que creo le va a interesar.

Como estamos arrancando en Culiacan, tenemos una promocion especial para los primeros 50 consultorios que se registren:

[DESCUENTO] en cualquier plan.

Eso significa que el plan [PLAN_RECOMENDADO] le quedaria en [PRECIO con descuento] al mes en lugar de [PRECIO].

Esta promocion es solo para consultorios que conocimos en persona, y se acaba cuando lleguemos a 50. Ya llevamos [X] registrados.

Le interesa que le aparte su lugar?
```

### Mensaje 4 - Dia 10 (Ultima oportunidad)
**Mejor horario:** Viernes, 11:00 AM-12:00 PM

```
[NOMBRE_DOCTOR], le escribo por ultima vez sobre esto.

La promocion de fundador esta por terminar y no queria que se la perdiera. Despues de esto el precio vuelve a la normalidad.

Si tiene alguna duda o algo que le detenga, digame con confianza. A veces es nada mas una pregunta la que falta resolver.

Y si de plano no le interesa, cero problema. Le agradezco su tiempo y aqui estaremos por si mas adelante le sirve.

Saludos,
[NOMBRE_VENDEDOR]
[TELEFONO_VENDEDOR]
```

---

# SECUENCIA 3: Post-Demo / Post-Trial Activo

> **Contexto:** El doctor ya vio la demo o se registro para el trial gratuito.

### Dia 1 - Bienvenida + Primeros pasos
**Mejor horario:** Enviar inmediatamente despues del registro o al dia siguiente a las 9:00 AM

```
Hola [NOMBRE_DOCTOR]! Bienvenido a DocFacil. Soy [NOMBRE_VENDEDOR] y voy a ser su contacto directo para lo que necesite.

Para que le saque el maximo provecho desde el inicio, estos son los 3 pasos que le recomiendo hacer hoy:

1. Agregar sus datos de doctor (nombre, cedula, especialidad) - toma 2 minutos
2. Registrar a 5 pacientes que tenga esta semana - puede copiarlos de su libreta
3. Agendar las citas de manana - y vea como les llega el recordatorio por WhatsApp

Si se atora en algo, me escribe por aqui y le ayudo al momento. Literal, contestamos en menos de 5 minutos.

Exito!
```

### Dia 3 - Tip #1: Agendar primera cita
**Mejor horario:** Martes a jueves, 9:00-10:00 AM

```
Hola [NOMBRE_DOCTOR], como le ha ido con DocFacil?

Tip rapido: cuando agende una cita, asegurese de poner el celular correcto del paciente. Asi el recordatorio por WhatsApp le llega automatico un dia antes.

Los doctores que usan los recordatorios nos reportan que sus faltas bajan entre 40-60%. Es lo que mas impacto tiene.

Ya pudo agendar su primera cita? Si necesita ayuda, aqui estoy.
```

### Dia 7 - Tip #2: Activar recordatorios WhatsApp
**Mejor horario:** Lunes o martes, 10:00-11:00 AM

```
[NOMBRE_DOCTOR], va una semana con DocFacil. Como va?

Si todavia no ha activado los recordatorios por WhatsApp, este es el momento. Es la funcion que mas dinero le ahorra porque recupera las citas que normalmente se pierden.

Para activarlos:
1. Vaya a Configuracion > WhatsApp
2. Active "Recordatorios automaticos"
3. Listo. Cada paciente recibe un mensaje un dia antes de su cita.

Si quiere le ayudo a configurarlo, me marca al [TELEFONO_VENDEDOR] y lo hacemos juntos en 3 minutos.
```

### Dia 10 - Caso de exito / ROI
**Mejor horario:** Miercoles o jueves, 1:00-2:00 PM

```
[NOMBRE_DOCTOR], a estas alturas ya deberia estar viendo resultados. Le comparto numeros reales:

Un consultorio dental en Culiacan con DocFacil:
- Antes: 6-8 citas perdidas por semana
- Ahora: 1-2 citas perdidas por semana
- Recupero: ~$8,000/mes en consultas que antes perdia
- Paga: $149/mes por DocFacil

Retorno: 50x lo que invierte.

Usted como va? Ha notado alguna diferencia? Me gustaria saber su experiencia.
```

### Dia 12 - Oferta fundador / descuento
**Mejor horario:** Martes a jueves, 11:00 AM-12:00 PM

```
[NOMBRE_DOCTOR], su periodo de prueba esta por terminar y quiero ofrecerle algo antes de que se acabe.

Como usted es de los primeros 50 consultorios en probar DocFacil, le tengo una promocion de fundador:

[DESCUENTO] los primeros 3 meses en el plan [PLAN_RECOMENDADO].

En lugar de [PRECIO], pagaria [PRECIO con descuento] al mes. Y se queda con todas las funciones activas.

Le interesa? Le activo el plan ahorita mismo.
```

### Dia 14 - Urgencia (Trial termina)
**Mejor horario:** El dia que termina el trial, 9:00-10:00 AM

```
[NOMBRE_DOCTOR], hoy es el ultimo dia de su prueba gratuita.

Si no activa un plan, manana perderia acceso a:
- Sus expedientes de pacientes
- Su historial de citas
- Los recordatorios automaticos por WhatsApp
- Sus recetas guardadas

No quiero que pierda todo lo que ya lleva avanzado.

Le puedo activar el plan [PLAN_RECOMENDADO] con la promocion de fundador ahorita mismo. Nada mas digame "si" y yo lo hago.

O si tiene alguna duda, con confianza.
```

---

# SECUENCIA 4: Reactivacion (Lead que dejo de responder)

### Mensaje 1 - 2 semanas despues del ultimo contacto
**Mejor horario:** Martes, 10:00-11:00 AM

```
Hola [NOMBRE_DOCTOR], soy [NOMBRE_VENDEDOR] de DocFacil. Platicamos hace un par de semanas sobre el sistema para su consultorio.

No le quito tiempo. Solo queria avisarle que sacamos una funcion nueva: ahora puede generar recetas PDF profesionales con sus datos, cedula y firma digital. Sus pacientes reciben la receta por WhatsApp.

Si le interesa verlo, le hago un demo rapido de 5 minutos. Si no, sin problema, no le vuelvo a escribir sobre esto.

Que me dice?
```

### Mensaje 2 - 1 mes despues
**Mejor horario:** Jueves, 1:00-2:00 PM

```
[NOMBRE_DOCTOR], espero que este muy bien. Le escribo rapido con un dato:

Ya somos [X] consultorios usando DocFacil en [Culiacan/Sinaloa/Mexico]. Los doctores nos dicen que en promedio ahorran 5-8 horas a la semana en temas administrativos.

Si su situacion cambio y ahora si le interesa organizar mejor su consultorio, el plan basico empieza en $149/mes y puede probarlo gratis primero.

[LINK_TRIAL]

Saludos.
```

### Mensaje 3 - 2 meses despues (Breakup)
**Mejor horario:** Viernes, 11:00 AM

```
[NOMBRE_DOCTOR], soy [NOMBRE_VENDEDOR] de DocFacil. Este es mi ultimo mensaje.

Entiendo que tal vez no era el momento o no era lo que necesitaba, y esta perfecto.

Nada mas le dejo mi contacto por si en el futuro le sirve: [TELEFONO_VENDEDOR]. Y el link de registro sigue activo si algun dia quiere probarlo: [LINK_TRIAL].

Le deseo mucho exito en su consultorio. Saludos.

- [NOMBRE_VENDEDOR]
```

---

# SECUENCIA 5: Email Frío Automático (pipeline real)

> **Importante:** Esta secuencia es la que el cron `docfacil:send-prospect-emails` manda automáticamente cada hora. Son **3 correos** (no 4), separados por 3 días, con pipeline de status `new → contacted → interested → lost`. Filtro del cron: `source='prospecting'`. Los templates reales viven en `app/Mail/Prospect*Mail.php` y `resources/views/emails/prospect-*.blade.php`. Si los editas, edita ambos lugares para mantener consistencia.
>
> Versión anterior (beta-framing) archivada en git history.

### Email 1 — Intro humanizada (status `new` → `contacted`)

**Asunto:** `Dr. [NOMBRE], ¿cuántos le dejaron plantado esta semana?`
**Blade:** `resources/views/emails/prospect-beta-invite.blade.php`
**Mail:** `App\Mail\ProspectBetaInviteMail`

Abre con una escena reconocible (agenda a las 10, paciente que no llegó, hueco). Pasa a beneficios concretos (recuperar $6-10k, ahorrar 20-30 min, bajar la carga de la recepcionista). Cierra con trial de **15 días sin tarjeta** + fallback a plan gratis permanente. CTA: "Probarlo 15 días gratis" o respuesta al correo para demo.

### Email 2 — Caso real (3 días después, status `contacted` → `interested`)

**Asunto:** `Dr. [NOMBRE], le dejo un caso que quizá le suene familiar`
**Blade:** `resources/views/emails/prospect-followup.blade.php`
**Mail:** `App\Mail\ProspectFollowupMail`

Storytelling: un dentista en Culiacán que pasó de 6-8 faltas/semana a 1-2 en 2 meses. Recupera ~$8k/mes pagando $499. Explica el *cómo* (WhatsApp el día anterior, botón para reagendar). CTA mismo trial 15 días.

### Email 3 — Cierre respetuoso (3 días después, status `interested` → `lost`)

**Asunto:** `Dr. [NOMBRE], cierro este hilo — gracias por su tiempo`
**Blade:** `resources/views/emails/prospect-last-chance.blade.php`
**Mail:** `App\Mail\ProspectLastChanceMail`

Respeta el "no" implícito. Deja 3 puertas abiertas: link de registro sin urgencia falsa, WhatsApp personal de Omar para cualquier duda (incluso opinar sobre otro software), petición de referido con reward (un mes gratis por colega que se suscriba).

---

### Templates WhatsApp paralelos (fallback si no hay email)

Cuando el prospecto tiene `phone` pero no `email`, el mismo cron manda por WhatsApp con el mismo arco:

1. **`prospect_beta_invite`** — Escena del hueco en la agenda + 3 beneficios + trial 15 días.
2. **`prospect_followup`** — Caso del dentista de Culiacán, recuperó $8k/mes.
3. **`prospect_last_chance`** — Despedida respetuosa + petición de referido.

Los templates están en `app/Console/Commands/SendProspectEmails.php:29-35` como constante `WA_MESSAGES`.

---

### Notas operacionales

- **Cron:** `Schedule::command('docfacil:send-prospect-emails')->hourly()` en [routes/console.php:15](routes/console.php).
- **Rate limit:** máximo 10 mensajes por corrida (protege WhatsApp de marcarte como spam).
- **Deduplicación:** el cron checa `lifecycle_emails` para no repetir el mismo mensaje al mismo prospect.
- **Canal:** prueba email primero; si no hay, WhatsApp. Nunca ambos al mismo prospect en el mismo paso.
- **Espacio entre mensajes:** 3 días (`DAYS_BETWEEN_MESSAGES = 3`).

---

# BONUS: Mensajes para Pedir Referidos a Clientes Activos

### Referido 1 - Despues de 30 dias como cliente
**Mejor horario:** Viernes, 12:00-1:00 PM

```
[NOMBRE_DOCTOR], ya lleva un mes con DocFacil y espero que le este funcionando bien.

Tengo una pregunta: conoce a algun colega doctor o dentista que tambien batalle con las citas perdidas o con organizar su consultorio?

Si me recomienda a alguien y se registra, le doy un mes gratis a usted y un descuento especial a su colega. Ganamos todos.

Solo me pasa el nombre y numero, y yo me encargo.
```

### Referido 2 - Despues de un caso de exito
**Mejor horario:** Cuando el doctor comparta algo positivo sobre el sistema

```
[NOMBRE_DOCTOR], me da mucho gusto que le este funcionando tan bien DocFacil.

Oiga, seria mucho pedirle que le cuente a algun colega? Muchos doctores todavia no saben que existe algo asi y siguen batallando con libretas y Excel.

Si alguien se registra por su recomendacion, le regalo un mes de su plan. Nada mas me dice el nombre y yo le escribo.
```

### Referido 3 - Recomendacion a cambio de upgrade
```
[NOMBRE_DOCTOR], le tengo una propuesta:

Si me recomienda con 3 colegas (no importa si contratan o no, nada mas que acepten ver la demo), le hago un upgrade gratis al plan [PLAN SUPERIOR] por 2 meses.

Solo necesito nombre, especialidad y numero de WhatsApp. Yo hago todo el trabajo.

Le late?
```

### Referido 4 - Referido natural en conversacion
```
Oiga [NOMBRE_DOCTOR], y en el edificio donde tiene su consultorio, hay otros doctores? Porque el sistema les podria servir igual que a usted.

Si me da chance de ir un dia y presentarme, le doy un mes gratis por la recomendacion.
```

### Referido 5 - Referido post-testimonial
```
[NOMBRE_DOCTOR], mil gracias por su comentario sobre DocFacil. Eso nos ayuda muchisimo.

Ultima cosa: si conoce a alguien que le pueda servir, me lo pasa y yo me encargo. Sin que usted tenga que hacer nada. Y como agradecimiento, le doy un [beneficio: mes gratis, descuento, feature extra].
```

---

# BONUS: Mensajes para Pedir Resenas/Testimonios

### Testimonio 1 - Solicitud directa despues de 45-60 dias
**Mejor horario:** Jueves o viernes, 12:00-1:00 PM

```
[NOMBRE_DOCTOR], ya lleva casi 2 meses con DocFacil y me gustaria pedirle un favor rapido.

Nos ayudaria mucho si nos pudiera dar un testimonio corto sobre su experiencia. Puede ser por aqui mismo, por audio de WhatsApp, o si prefiere lo escribo yo y usted nada mas lo aprueba.

Algo asi como:
- Que problema tenia antes?
- Que le ha funcionado mas de DocFacil?
- Se lo recomendaria a un colega?

Su testimonio nos ayuda a que mas doctores confien en el sistema. Se lo agradeceria mucho.
```

### Testimonio 2 - Version facil (solo aprobacion)
```
[NOMBRE_DOCTOR], le escribo algo rapido. Basado en lo que me ha platicado, redacte este testimonial para nuestra pagina:

"Antes perdia varias citas a la semana porque los pacientes no llegaban. Con DocFacil, los recordatorios por WhatsApp bajaron mis faltas de 6 a 1 por semana. Ademas ya tengo todos mis expedientes organizados y las recetas las hago en segundos."
- [NOMBRE_DOCTOR], [ESPECIALIDAD] en Culiacan

Esta bien si lo usamos? Si quiere cambiarle algo, con toda confianza.
```

### Testimonio 3 - Video corto
```
[NOMBRE_DOCTOR], nos ayudaria muchisimo con un video corto (30-40 segundos) contando su experiencia con DocFacil.

No tiene que ser nada producido, desde su celular esta perfecto. Solo 3 cosas:
1. Como se llama y que especialidad tiene
2. Que problema le resolvio DocFacil
3. Si se lo recomendaria a un colega

Si le late, le mando las preguntas exactas para que sea super facil.

Y como agradecimiento, le doy [beneficio: mes gratis / feature extra].
```

---

# BONUS: Script de Llamada Telefonica (60 segundos)

> **Contexto:** Para cuando el vendedor llama por telefono a un prospecto frio o de seguimiento.

```
[Tono amable, ritmo natural, NO leer como robot]

"Hola, buenas tardes. Hablo con [NOMBRE_DOCTOR]?

... [esperar confirmacion]

Doctor, mucho gusto. Mi nombre es [NOMBRE_VENDEDOR], le llamo de DocFacil, 
somos un software para consultorios medicos y dentales.

Le quito solo un minuto. La razon de mi llamada es que estamos trabajando
con varios [dentistas/medicos] aqui en Culiacan, y el problema que mas nos 
reportan es el de las citas perdidas: pacientes que no llegan sin avisar.

Lo que hace DocFacil es que le manda un recordatorio automatico por WhatsApp 
a cada paciente un dia antes de su cita. Los doctores que lo usan nos dicen 
que sus faltas bajaron entre un 40 y 60 por ciento.

Ademas le organiza expedientes, agenda y le genera recetas PDF.

El plan basico esta en $149 al mes y puede probarlo gratis.

Doctor, tiene 10 minutos esta semana para que se lo muestre? Puede ser en 
su consultorio o por videollamada, lo que le quede mejor.

... [esperar respuesta]

[Si SI:] Perfecto, que dia y hora le funciona? Le mando la confirmacion 
por WhatsApp.

[Si NO / NO ME INTERESA:] Entiendo perfectamente, doctor. Le puedo mandar 
la info por WhatsApp por si mas adelante le interesa? Es sin compromiso.

[Si ESTOY OCUPADO:] Claro, que dia y hora le queda mejor para que le marque? 
Son nada mas 10 minutos.

Muchas gracias por su tiempo, [NOMBRE_DOCTOR]. Que le vaya muy bien."
```

---

# TIPS GENERALES PARA VENDEDORES

1. **Mejor dia para primer contacto:** Martes a jueves. Lunes estan saturados, viernes ya estan pensando en el fin de semana.

2. **Mejor hora para WhatsApp:** 1:00-2:30 PM (hora de comida entre pacientes) o 6:00-7:00 PM (ya cerraron consultorio).

3. **Mejor hora para llamar:** 10:00-11:00 AM o 1:00-2:00 PM.

4. **Mejor hora para emails:** 8:00-9:00 AM (lo ven al abrir correo).

5. **Nunca manden mas de 3 mensajes frios sin respuesta.** Respeten el "no" implicito y pasen a reactivacion despues de 2 semanas.

6. **Personalicen siempre.** Un "[NOMBRE_DOCTOR]" generico se siente frio. Si saben algo del consultorio (la colonia, si es consultorio nuevo, cuantos doctores tienen), mencionenlo.

7. **El follow-up gana.** El 80% de las ventas se cierran entre el contacto 5 y 12. No se rindan en el mensaje 2.

8. **WhatsApp > Email > Llamada** en tasa de respuesta para este mercado.

9. **Siempre cierren con una pregunta** o un CTA claro. Nunca terminen un mensaje en el aire.

10. **El precio de $149 se vende solo si primero venden el problema.** Siempre hablen primero de citas perdidas y dinero que se va, luego de la solucion, y al final del precio.
